...
import { LogEntry, ProfileSettings, Language } from '../types';
...
const TimeTracker: React.FC<TimeTrackerProps> = ({ addLog, profile, t, showToast, language }) => {
  ...
  return (
    <div className="bg-glass-bg-light dark:bg-glass-bg-dark backdrop-blur-xl border border-glass-border-light dark:border-glass-border-dark p-6 sm:p-8 rounded-2xl shadow-2xl shadow-shadow-color-light dark:shadow-shadow-color-dark w-full max-w-4xl mx-auto transition-all duration-300">
      ...
      <div className="flex flex-col md:flex-row justify-center items-center my-8 gap-4 sm:gap-8">
        {/* Clock In Button */}
        <button
          onClick={handleStart}
          disabled={isRunning}
        ...
        {/* Day Info */}
        <div className="order-first md:order-none flex flex-col items-center justify-center text-center p-4">
            <p className="text-3xl font-bold text-gray-800 dark:text-white">{dayName}</p>
            <p className="text-sm text-gray-500 dark:text-gray-400">{fullDate}</p>
            {isWeekend && !isRunning && (
                <div className="mt-4 px-3 py-1 text-xs font-semibold text-amber-800 bg-amber-100 dark:text-amber-200 dark:bg-amber-900/50 rounded-full whitespace-nowrap">
                    {t('weekendWarning')}
                </div>
            )}
        </div>
