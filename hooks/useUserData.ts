...
import { LogEntry, ProfileSettings, Task, Holiday } from '../types';
  ...
  useEffect(() => {
    if (user) {
      ...
      const unsubscribeProfile = userDocRef.onSnapshot((doc: any) => {
        if (doc.exists) {
          const data = doc.data();
          // Basic migration for old string[] holidays format
          if (data.officialHolidays && data.officialHolidays.length > 0 && typeof data.officialHolidays[0] === 'string') {
            data.officialHolidays = data.officialHolidays.map((dateStr: string) => ({ date: dateStr, name: 'Holiday', imported: false }));
          }
      ...
      // Basic migration for guest data
      const currentLocalProfile = localProfile;
      if (currentLocalProfile.officialHolidays && currentLocalProfile.officialHolidays.length > 0 && typeof (currentLocalProfile.officialHolidays as any)[0] === 'string') {
          const migratedHolidays = (currentLocalProfile.officialHolidays as any as string[]).map(dateStr => ({ date: dateStr, name: 'Holiday', imported: false }));
          setLocalProfile({ ...currentLocalProfile, officialHolidays: migratedHolidays });
      }
